...
public class Estoque {
	...
	public void removerLista(String nomeProduto) {
		for (int i = 0; i < estoque.size(); i++) {
			if (estoque.get(i).p.marca.equalsIgnoreCase(nomeProduto)) {
				estoque.remove(i);
			}
		}
	}
	...
	public void alterarPreco(String nome, int opcao, double novoPreco, int novaQuantidade) {
		for (int i = 0; i < estoque.size(); i++) {
			...
			if (estoque.get(i).p.marca.equalsIgnoreCase(nome)) {
				// caso queira alterar preco digita 1
				if (opcao == 1) {
					estoque.get(i).p.preco = novoPreco;
				// caso queira alterar a quantidade digita 2
				} else if (opcao == 2) {
					estoque.get(i).quantidade = novaQuantidade;
				}
			}
		}
	}
	...
	public void visualizarLista() {
		// Verificar Lista
		for (int i = 0; i < estoque.size(); i++) {
			System.out.printf("Produto: %s\nMarca: %s\nPreco: %.2f\nQuantidade: %dunid\n\n", estoque.get(i).p.tipo, estoque.get(i).p.marca,
					estoque.get(i).p.preco, estoque.get(i).quantidade);
